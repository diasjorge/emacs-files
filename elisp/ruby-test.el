...
filename or the given filename."
  (let ((filename (or filename (buffer-file-name))))
    (cond ((not filename)
           nil)
          ((string-match "\\(.*\\)\\(spec/\\)\\(controllers\\|helpers\\|models\\|views\\|mailers\\)\\(.*\\)\\([^/]*\\)\\(_spec\\)\\(\\.rb\\)$" filename)
...
filename or the given implementation filename."
  (let ((implementation-filename (or implementation-filename (buffer-file-name))))
    (cond ((not implementation-filename)
           nil)
          ((string-match "\\(.*\\)\\(app/\\)\\(controllers\\|helpers\\|models\\|views\\|mailers\\)\\(.*\\)\\([^/]*\\)\\(\\.rb\\)$" implementation-filename)
...
filename or the given implementation filename."
  (let ((implementation-filename (or implementation-filename (buffer-file-name))))
    (cond ((not implementation-filename)
          ...
          ((string-match "\\(.*\\)\\(app/\\)\\(models\\|mailers\\)\\(.*\\)\\([^/]*\\)\\(\\.rb\\)$" implementation-filename)
